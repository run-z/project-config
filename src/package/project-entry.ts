...
export abstract class ProjectEntry extends ProjectDevTool {
   ...
   *
   * `false` when {@link sourceFile} is not set.
   */
  ...
  async #isGenerated(): Promise<boolean> {
    const sourceFile = await this.sourceFile;
   ...
   * Short entry name.
   *
   * By default, equals to {@link distFile distribution file} path relative to {@link ProjectOutput#distDir
  ...
  protected async detectName(): Promise<string> {
  ...
  protected async detectSourceFile(): Promise<string | null> {
    const { distDir } = await this.project.output;
    const distFile = path.relative(distDir, path.resolve(distDir, await this.distFile));
   ...
   */
  abstract get distFile(): Promise<string>;
  ...
  protected async detectTypesFile(): Promise<string> {
